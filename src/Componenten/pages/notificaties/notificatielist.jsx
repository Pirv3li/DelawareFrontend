    ...
    const fetchData = async () => {
        try {
            if (localStorage.getItem('roles') == 'leverancier') {
                ...
                const response = await getById(`notificatie/leverancier/${idLeverancier}`);
                setItems(response);
            }
                ...
                <Table colorScheme="Gray 500" mt={10} mb={5}>
                    ...
                    <Tbody>
                        {items.map((item) => (
                            ...
                            <Tr key={item.idNotificatie} onClick={() => handleClick(item)} borderBottom="1px solid" borderColor="gray.200" _hover={{ bg: hoverColor }}>
